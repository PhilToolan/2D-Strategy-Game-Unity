...
public class Barracks : MonoBehaviour
{
    ...
    {
        menu.SetActive(!menu.activeSelf);//if menu is activated it will deactive and vice versa
    }
    ...
    public void BuyItem(BarracksItem item )
    {
        if (gm.playerTurn == 1 && item.cost <= gm.player1Gold)//can player afford item
        {
            gm.player1Gold -= item.cost;
            player1Menu.SetActive(false);
        }
        else if (gm.playerTurn == 2 && item.cost <= gm.player2Gold)//can player afford item
        {
